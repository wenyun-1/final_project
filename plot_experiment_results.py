...
def _load_chapter3_curve_data(soh_output: str) -> Dict[str, pd.DataFrame]:
    """优先读取点级结果；若不存在，则用 Pred_SOH + pseudo-label 进行对齐。"""
    ...
    need_pred = {"Vehicle", "Days", "Pred_SOH"}
    if not need_pred.issubset(pred.columns):
        return {}
    ...
    if pred["Pred_SOH"].max() <= 1.5:
        pred["SOH_pred"] = pred["Pred_SOH"] * 100.0
    else:
        pred["SOH_pred"] = pred["Pred_SOH"]

    pseudo_files = glob.glob(os.path.join(soh_output, "*_pseudo_labels.csv"))
    ...
    for f in pseudo_files:
        ...
        if {"days", "soh_true"}.issubset(d.columns):
            ...
            d = d.dropna().sort_values("Days")
            pseudo_map[veh] = d

    veh_data: Dict[str, pd.DataFrame] = {}
    for veh, g in pred.groupby("Vehicle"):
        gp = g[["Vehicle", "Days", "SOH_pred"]].sort_values("Days").copy()
        veh = str(veh)
        if veh in pseudo_map and not pseudo_map[veh].empty:
            merged = pd.merge_asof(
                gp.sort_values("Days"),
                pseudo_map[veh].sort_values("Days"),
                on="Days",
                direction="nearest",
            )
        else:
        ...
        veh_data[veh] = merged[["Days", "SOH_true", "SOH_pred"]].dropna(subset=["Days", "SOH_pred"])
...
def plot_soh_metric_bars(metrics_vehicle_csv: str, out_dir: str) -> Optional[str]:
    ...
    axes[0].bar(x - w / 2, df["RMSE_raw"], width=w, color="#4e79a7", label="Raw")
    axes[0].bar(x + w / 2, df["RMSE_filtered"], width=w, color="#e15759", label="Filtered")
    axes[0].set_title("RMSE Comparison by Vehicle")
    axes[0].set_xticks(x)
    ...
    axes[1].bar(x - w / 2, df["MAE_raw"], width=w, color="#4e79a7", label="Raw")
    axes[1].bar(x + w / 2, df["MAE_filtered"], width=w, color="#e15759", label="Filtered")
    axes[1].set_title("MAE Comparison by Vehicle")
    axes[1].set_xticks(x)
...
def plot_ablation(ablation_csv: str, out_dir: str) -> Optional[str]:
    ...
    required = {"tag", "RMSE_raw_mean", "RMSE_filtered_mean", "R2_raw_mean", "R2_filtered_mean"}
    ...
    axes[0].bar(x - w / 2, df["RMSE_raw_mean"], width=w, color="#4e79a7", label="Raw")
    axes[0].bar(x + w / 2, df["RMSE_filtered_mean"], width=w, color="#e15759", label="Filtered")
    ...
    axes[1].bar(x - w / 2, df["R2_raw_mean"], width=w, color="#4e79a7", label="Raw")
    axes[1].bar(x + w / 2, df["R2_filtered_mean"], width=w, color="#e15759", label="Filtered")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="绘制第三章SOH与第四章SOC对比图")
